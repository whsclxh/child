...
$sql7 = "SELECT * FROM users where total_times>5 ORDER BY total_times+0 DESC";
$result7 = mysqli_query($qaq,$sql7);
$total_ = mysqli_num_rows($result7);
$sql8 = "SELECT * FROM users where total_times>5 ORDER BY total_average+0 DESC";
$result8 = mysqli_query($qaq,$sql8);
$total_25 = mysqli_num_rows($result8);
?>
<!DOCTYPE html>
<html lang="en">
...
<script type="text/javascript" src="jquery-3.3.1.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        window.onload=function(){
            ...
            //上面确定了 现在开始创建
            <?php for($x=1;$x<= $total_;$x++){ ?>
            <?php $row = mysqli_fetch_array($result7);?>
            <?php if($row[0]==$id){ ?>
                document.getElementById("you").innerHTML='<?php echo "你的排名為<font color=\"red\">$x</font>/$total_" ?>';
            <?php } ?>
            ...
            tdNode3.innerHTML='<?php echo "$row[14]"; ?>';
            var tdNode4=trNode.insertCell();
            tdNode4.innerHTML='<?php echo "$row[9]"; ?>';
            <?php } ?>
        ...
        $("table td:last-child, table th:last-child").addClass("last");
            ...
            //上面确定了 现在开始创建
            <?php for($u=1;$u<= $total_25;$u++){ ?>
            <?php $rowy = mysqli_fetch_array($result8);?>
            <?php if($rowy[0]==$id){ ?>
                document.getElementById("you2").innerHTML='<?php echo "你的排名為<font color=\"red\">$u</font>/$total_25" ?>';
            <?php } ?>
            ...
            tdNode32.innerHTML='<?php echo "$rowy[14]"; ?>';
            var tdNode42=trNode2.insertCell();
            tdNode42.innerHTML='<?php echo "$rowy[9]"; ?>';
            <?php } ?>
